...
export default function Login() {
    ...
    const handleSubmit = () => {
        axios
            .post(`https://penarol-app.herokuapp.com/api/user/${values.email}`, values, { withCredentials: true })
            .then(res => {
                if (res.data.msg === 'Successfully connected') {
                    history.push("/history");
                    cookies.set('token', res.data.token, { path: '/', maxAge: 6000000, secure: true});
                }
            })
